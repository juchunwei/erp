...
- 管理->设置中, 可以设置班次和工作时间. 如下图, 表示分2个班次,第一个班次8:00-11:30,12:00-17:00, 第二个班次17:00-20:30,21:00-23:00
- 汇报开始时, 作业的结束时间默认为本班次的下班时间
- 汇报作业时, 根据设置的班次自动计算`作业记录`的`小时`
- 参数: `汇报时必须汇报作业开始`, 启用该选项, 在汇报产量的时候, 输入员工号后, 不需要输入生产单号, 系统自动带出该员工已经开始的作业(作业日记账没有过账, 并且没有`合格数量`的行), 更改完成数量, 提交即可. 如果不输入完成数量, 则只更新时间 
